...
function move(currentPosition, moveInstruction) {
  let result = [];
  for (let i = 0; i < Number(moveInstruction.slice(1)); i++) {
...
for (let i = 0; i < wires.length; i++) {
  const moves = wires[i].split(",");

  let currentPosition = [0, 0];
  arrayPositions[i] = [];
  for (let j = 0; j < moves.length; j++) {
    let [position, result] = move(currentPosition, moves[j]);
    arrayPositions[i] = [...arrayPositions[i], ...result];
    currentPosition = position;
  }
}
let stringPositions = arrayPositions.map(wire =>
  wire.map(pos => `${pos.join(",")}`)
);
let intersections = [];
let firstIntersection;
for (let i = 0; i < stringPositions.length; i++) {
  for (let j = i + 1; j < stringPositions.length; j++) {
    // firstIntersection = stringPositions[i].find(pos =>
    //   stringPositions[j].includes(pos)
    // );
    intersections = [
      ...intersections,
      ...stringPositions[i].filter(pos => stringPositions[j].includes(pos))
    ];
  }
}
...
for (let m = 0; m < intersections.length; m++) {
  ...
  for (let i = 0; i < arrayPositions.length; i++) {
    // console.log(intersections[m]);
    const moves = arrayPositions[i].slice(
      0,
      stringPositions[i].indexOf(intersections[m]) + 1
    );

    // console.log(moves.length);
    steps = steps + moves.length;
  }
  stepsArray.push(steps);
}
console.log(Math.min(...stepsArray));
return;
let distances = intersections.map(point => {
  point = point.split(",");
  return Math.abs(point[0] - 0) + Math.abs(point[1] - 0);
});

console.log(distances);

console.log(Math.min(...distances));
// let result = stringPositions[0].filter(pos => stringPositions[1].includes(pos));
// result = result.map(point => point.split(","));
// console.log(result);
// let initPos = [0, 0];

// let distances = result.map(
//   point => Math.abs(point[0] - 0) + Math.abs(point[1] - 0)
// );

// console.log(Math.min(...distances));

// console.log(move([0, 0], "D10"));
